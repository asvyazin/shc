...
class MaxVersionsSuite extends SHC with Logging {
  ...
  def withCatalog(cat: String, options: Map[String,String]): DataFrame = {
    sqlContext.read
      .options(options ++ Map(HBaseTableCatalog.tableCatalog -> defineCatalog(tableName)))
  ...
  test("Max Versions semantics") {
    ...
    persistDataInHBase(defineCatalog(tableName), oldestData, oldestMs)
    persistDataInHBase(defineCatalog(tableName), oldData, oldMs)
    persistDataInHBase(defineCatalog(tableName), newData, newMs)
    persistDataInHBase(defineCatalog(tableName), newestData, newestMs)

    // Test specific last two versions
    val twoVersions: DataFrame = withCatalog(defineCatalog(tableName), Map(
    ...
    val threeVersions: DataFrame = withCatalog(defineCatalog(tableName), Map(
    ...
    val lastVersions: DataFrame = withCatalog(defineCatalog(tableName), Map.empty)
